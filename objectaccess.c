...
void PutField(contextDef* context, jobject obj, u2 address, stackable* value) {
    HEAP_VALIDATE;

    if (obj != NULL) {
        stackable* objectPayload = (stackable*) GetObjectPayload(context, obj);
...
stackable* GetField(contextDef* context, jobject obj, u2 address) {
    HEAP_VALIDATE;

    stackable* objectPayload;
    if (obj != NULL) {
        objectPayload = (stackable*) GetObjectPayload(context, obj);
        objectPayload += address;
    } else {
        objectPayload = NULL;
        throwNullPointerException(context);
 ...
 */
jobject newObject(contextDef* context, u2 classId) {
    // TODO Move to jni.c and add correct semantics (by calling the constructor method as well)
    u2 size;
    getClassSize(classId, &size);

    jobject jref = heapAllocObjectByStackableSize(context, size, classId);
